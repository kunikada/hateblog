	...
	<div class="inner">
		<div id="content">
			<h2 class="archiveheader">
                <span class="headerleft"><?php echo $left_title ?></span>
                <span class="headerright"><?php echo $right_title ?></span>
            </h2>
<?php echo $pagination ?>
			<div class="clearfix">
...
<?php foreach ($keyphrases[$row['id']] as $word): ?>
                    <a href="/tag/<?php echo rawurlencode($word) ?>/" title="タグ「<?php echo $word ?>」 - <?php echo $site_name ?>" class="tagword"><?php echo $word ?></a>
<?php endforeach ?>
                </p>
...
<?php endforeach ?>
			...
			<?php if ($year > 2010): ?>
            <p class="postmeta">ランキング: <a href="/ranking/<?php echo "$year/$month_week" ?>/" title="<?php echo $tag_title ?> - <?php echo $site_name ?>"><?php echo is_null($week) ? '週間' : '月間' ?>ランキング</a></p>
			<?php endif; ?>
<?php echo $pagination ?>
        </div><!-- /#content -->
        <div id="sidebar">
