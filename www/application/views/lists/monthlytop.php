...
<?php if ($i == $top_count): ?>
                    <span class="barsolo">月間ランキング TOP <?php echo $i ?></span>
<?php else: ?>
                    <a class="barsolo" href="/ranking/monthlytop<?php echo $i ?>/<?php echo $year ?>">月間ランキング TOP <?php echo $i ?></a>
<?php endif ?>
<?php endforeach ?>
                </div>
                <ul class="level1">
<?php foreach ($data1['lists1'] as $mm => $data2): ?>
                    <li><a href="/ranking/<?php echo "{$year}/{$mm}" ?>/" title="月間ランキング <?php echo "{$year}年{$mm}月" ?> - <?php echo $site_name ?>"><?php echo $mm ?>月 月間ランキング</a></li>
                    <ul class="level2">
<?php foreach ($bookmarks[$mm] as $row): ?>
                        <li>
