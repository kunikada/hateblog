	...
	<div class="inner">
		<div id="content">
			<h2 class="archiveheader">
                <span class="headerleft"><a href="/hotlist/" title="<?php echo "{$main_title} - {$site_name}" ?>">人気エントリー</a></span>
...
<?php endforeach ?>
			...
			<p class="postmeta">アーカイブ: <a href="/archive/<?php echo $str_date ?>/" title="<?php echo "アーカイブ {$year}年{$month}月 - {$site_name}" ?>"><?php echo "{$year}年{$month}月" ?></a> ランキング: <a href="/ranking/<?php echo "{$year}/{$month}" ?>/" title="<?php echo "{$year}年{$month}月 月間ランキング - {$site_name}" ?>">月間ランキング</a>, <a href="/ranking/<?php echo "{$year}/{$str_week}" ?>/" title="<?php echo "週間ランキング {$sub_title} - {$site_name}" ?>">週間ランキング</a></p>
<?php echo $pagination ?>
		</div><!-- /#content -->
        <div id="sidebar">
<?php echo $sidebar ?>
