	...
	<div class="inner">
		<div id="content">
			<h2 class="archiveheader">
                ...
                <span class="headerright"><?php echo htmlspecialchars(mb_strimwidth($sub_title, 0, 36, '...')) ?></span>
            </h2>
...
?>
                <h3 style="background-image:url(/favicons?domain=<?php echo $row['domain'] ?>)"><a class="<?php echo $visited ?>" href="<?php echo $row['url'] ?>" title="<?php echo htmlspecialchars($row['title']) ?>" target="_blank"<?php echo $onclick ?>><?php echo preg_replace('/('.$highlight_word.')/iu', '<span class="highlight">$1</span>', htmlspecialchars($row['title'])) ?></a></h3>
                <p class="timestamp">
...
<?php endif ?>
                <p class="framed<?php if ($row['screenshot']) echo ' withscreenshot' ?>"><?php echo preg_replace('/('.$highlight_word.')/iu', '<span class="highlight">$1</span>', htmlspecialchars($row['description'])) ?> <a href="<?php echo $row['url'] ?>" title="<?php echo htmlspecialchars($row['title']) ?>" class="continued <?php echo $visited ?>"<?php echo $onclick ?>>続きを読む</a></p>
<?php endforeach ?>
			</div><!-- /.clearfix -->
<?php echo $pagination ?>
        </div><!-- /#content -->
